...
import com.yunqi.fish.common.constant.CommonConstant;
...
import static com.yunqi.fish.common.constant.RedisKeyConstant.DELETE_REDISSON_LOCK;
...
public class DeleteFileJob {
    ...
    public void deleteFileByTime() {
        //加个锁保险起见
        RLock lock = redissonClient.getLock(DELETE_REDISSON_LOCK);
        try {
            //尝试加锁,加锁成功的操作
            if (lock.tryLock(100, TimeUnit.SECONDS)) {
                try {
                    ...
                    //算法运行结果文件
                    File file1 = new File(CommonConstant.ALGORITHM_RESULT_DELETE);
                    deleteFile(file1);

                    Integer count = moveFileByDate(CommonConstant.NGINX_PATH, 30);
    ...
    public void deleteFileByMemory() {
        File file = new File(CommonConstant.NGINX_PATH);
        file.getUsableSpace();
        // 可用剩余空间
        long usableSpace = file.getUsableSpace() / 1024 / 1024 / 1024;
        if (usableSpace <= 10) {
            Integer count = moveFileByDate(CommonConstant.NGINX_PATH, 3);
